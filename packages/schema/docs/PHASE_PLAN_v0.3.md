...
3. **Semantic-loss metadata** — when JSON Schema can accept the same input but cannot express a NekoStack runtime action or annotation directly, the output emits `x-nekostack-*` extension keys rather than silently dropping the information. Examples: `default()` emits `x-nekostack-default-applied-by: "runtime"` (Decision #9); `stripUnknown` emits `x-nekostack-strip: true` (Decision #12). **Runtime refinements** and **regex refinements with non-empty flags** are NOT metadata cases in v0.3 — they would change validation behavior, so they throw `UnsupportedNodeKindError` (Decisions #11 and #11a). Metadata is for "JSON Schema accepts the same thing, but downstream consumers need to know there's more going on"; throw is for "emitting this would change what the schema validates."
