...
- **`planMigration({ schemaRegistry, migrationRegistry, schemaId, fromVersion, toVersion })`** — diff-aware planner. Consumes **both** registries so it can honor Decision #10's severity → migration-requirement dispatch (no migration needed for `null` / `cosmetic`; chain required for `breaking`; `over_specified` plan-note for `additive`). DFS chain enumeration over the `(schemaId, fromVersion)` adjacency. Never calls `transform`.
- **`verifyMigrationProvenance(registry)`** — four-way verdict matrix mirroring v0.7 freshness: `bound` / `cosmetic_drift` / `drift` / `missing_endpoint`. `cosmetic_drift` is warning-class (success branch); the others are failure-class with stable metadata reasons. Iteration sorted by `(schemaId, fromVersion, toVersion)` for determinism.
