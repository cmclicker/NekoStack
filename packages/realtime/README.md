...
| | |
|---|---|
| **Build tier** | Project unblocker â€” Leytide multiplayer can't proceed without it |
...
| **Est. to v1.0** | 16â€“32 weeks focused (real-time correctness is genuinely hard) |
...
â€¦then you need a persistent transport layer. The naive answer is "use Socket.io and pray." That works for a hello-world but breaks down quickly because:
...
5. There's no schema discipline by default â€” every project invents its own message shapes.
...
1. **Learning real-time correctness.** Reconnect logic, replay semantics, backpressure, presence â€” all subtle CS that pays off forever.
...
> See [`BOUNDARIES.md`](../../BOUNDARIES.md) Â§31 for the full capability map.
...
- CRDT primitives for collaborative state (Yjs adapter â€” opt-in)
...
| Yjs / CRDT primitives themselves | external (Yjs â€” we provide the transport adapter) |
...
**Depends on:**
- `@nekostack/schema` â€” message shapes.
- `@nekostack/auth` â€” per-message authorization.
- `@nekostack/telemetry` â€” connection events, message metrics.
- (Optional) Redis or NATS for multi-process pub/sub.
- (Optional) Yjs for collaborative state.

**Used by:**
- **NekoVibe** â€” live leaderboard updates, "X just completed today's puzzle" notifications.
- **Leytide** â€” multiplayer chat, party state, world events, instance presence.
- **NekoBattler** â€” live spectator mode for shareable replays.
- **NekoSystems** â€” live tenant + workflow dashboards, live operations telemetry.
- Future Business-OS / retail-ops â€” live KPI dashboards, multi-user inventory operations.
...
â”œâ”€â”€ src/
â”‚   â”œâ”€â”€ server/
â”‚   â”‚   â”œâ”€â”€ server.ts         # WebSocket + SSE server
â”‚   â”‚   â”œâ”€â”€ channel.ts        # typed channel
â”‚   â”‚   â”œâ”€â”€ presence.ts
â”‚   â”‚   â”œâ”€â”€ replay.ts         # sequence-numbered replay buffer
â”‚   â”‚   â””â”€â”€ auth.ts           # per-message authorization hook
â”‚   â”œâ”€â”€ client/
â”‚   â”‚   â”œâ”€â”€ client.ts         # typed client with reconnect
â”‚   â”‚   â”œâ”€â”€ hooks.ts          # React hooks: useChannel, usePresence
â”‚   â”‚   â””â”€â”€ transport/
â”‚   â”‚       â”œâ”€â”€ ws.ts
â”‚   â”‚       â””â”€â”€ sse.ts        # fallback for restrictive networks
â”‚   â”œâ”€â”€ adapters/             # pub/sub backplane
â”‚   â”‚   â”œâ”€â”€ memory.ts
â”‚   â”‚   â”œâ”€â”€ redis.ts
â”‚   â”‚   â””â”€â”€ nats.ts
â”‚   â”œâ”€â”€ collab/               # optional CRDT layer
â”‚   â”‚   â””â”€â”€ yjs.ts
â”‚   â””â”€â”€ testing/
â”‚       â””â”€â”€ fixtures.ts       # in-memory transport for tests
â”œâ”€â”€ tests/
â””â”€â”€ README.md
```
...
### v0.1 â€” WebSocket server + typed channels
...
### v0.4 â€” Presence + rooms
- Room membership.
- Presence tracking with join/leave events.

### v0.5 â€” Pub/sub backplane
...
- **Estimated learning return:** Very high. WebSocket protocol semantics, reconnect/replay design, pub/sub patterns, CRDT theory â€” all transferable.
