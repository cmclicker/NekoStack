...
| Contract docs | Package-specific contracts that bind future work (`IR_CONTRACT.md`, `API_SHAPE.md`, `ERROR_CODES.md`, etc.). Zero or more, as the package needs. | Packages that bind downstream consumers need named, enforceable contracts beyond invariants — e.g., `@nekostack/schema`'s `IR_CONTRACT.md` constrains every future generator. |
